...
class DataContext:
    ...
    @classmethod
    def from_folder(cls, base_path: str | "os.PathLike[str]") -> "DataContext":
        base = pd.Path(base_path) if hasattr(pd, "Path") else base_path  # kaggle-safe fallback
        # If Path isn't available from pandas, user can use pathlib in notebook.
        raise NotImplementedError(
            "Use explicit construction in the notebook, e.g.\n"
            "ctx = DataContext(brands=brands, products=products, ...)"
        )
...
def build_brand_day_frame(
    ...
    This is the main input into the DataAnalystAgent.
    """
...
def summarize_brand_window(
    ...
    Example metrics:
      - avg / total qty_sold, revenue
      - avg facings, share_of_shelf, oos_rate
      - total STT mentions, avg sentiment
      - avg traffic, rainfall, temperature
    """
    df = feature_frame.copy()
    if "date" not in df.columns:
        raise ValueError("feature_frame must contain a 'date' column")

    # For capstone simplicity: just aggregate over entire frame (you can add real rolling later)
    grouped = (
        df.groupby("brand_id")
        .agg(
